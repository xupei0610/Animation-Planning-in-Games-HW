...
Item* Scene::add(std::shared_ptr<Item> obj)
{
...
void Scene::restart()
{
    ...
    geo_shader->set("headlight.diffuse", glm::vec3(.25f, .2f, .125f));
    geo_shader->set("headlight.specular", glm::vec3(.5f, .5f, .5f));
    geo_shader->set("headlight.coef_a0", 1.f);
    geo_shader->set("headlight.coef_a1", .09f);
    geo_shader->set("headlight.coef_a2", .032f);
    geo_shader->set("global_ambient", glm::vec3(.3f, .3f, .3f));
    geo_shader->activate(false);
