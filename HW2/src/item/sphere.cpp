...
ItemInfo item::Sphere::ITEM_INFO("Sphere", "", 0, false, false, true);

item::Sphere::Sphere(const glm::vec3 &p, const glm::vec3 &s,
...
unsigned int item::Sphere::vao = 0;
unsigned int item::Sphere::vbo[2] = {0, 0};
Shader *item::Sphere::shader = nullptr;

unsigned int item::Sphere::initShader(unsigned int n_theta)
{
    if (shader == nullptr)
    {
        constexpr auto vs =
...
void item::Sphere::destroyShader()
{
    glDeleteVertexArrays(1, &vao);
    glDeleteBuffers(2, vbo);
    delete shader;
...
std::size_t item::Sphere::regItem()
{
    if (ITEM_INFO.id() == 0)
    {
        Item::reg(item::Sphere::ITEM_INFO, item::Sphere::create);
        if (ITEM_INFO.id() == 0)
...
std::shared_ptr<Item> item::Sphere::create()
{
    return std::shared_ptr<Item>(new Sphere());
}

void item::Sphere::enlight(const Light *light)
{
    if (light == nullptr)
    {
        _lighting = false;
...
void item::Sphere::hit(glm::vec3 const &at, glm::vec3 const &norm)
{
...
void item::Sphere::init()
{
    _n_indices = initShader(grid());
}

void item::Sphere::render()
{
...
void item::Sphere::setGrid(unsigned int n)
{
    _n_theta = n;
}

void item::Sphere::blend(GLenum sfactor, GLenum dfactor)
{
    blend_s = sfactor;
    blend_d = dfactor;
}
